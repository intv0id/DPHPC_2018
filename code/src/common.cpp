...
compVertexFALvec::compVertexFALvec(union_find* u_): u(u_){}
...
bool compVertexFALvec::operator()(const component_FAL_vector* v1,const component_FAL_vector* v2) const {
	int p1 = u->find(v1->index);
	int p2 = u->find(v2->index);
	return p1 < p2;
}
...
void print_edge_vec(v_edge_t mst){
    int c = 0;
	cout << endl;
