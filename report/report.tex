...
The implementation of our algorithms is based upon the work in several previously published papers.
...
\mypar{Kruskal with parallel sorting} Kruskal's algorithm is inherently sequential as it linearly goes through the edges once these ones have been sorted. Thus, our first implementation of Kruskal's algorithm is the sequential algorithm described in Algorithm \ref{algo:kruskal} preceded by a parallel sorting using TBB (Section \ref{sec:exp}).
...
For the cost analysis of the algorithm, we use $T(m)$ for the runtime of the algorithm with m edges. The choosing of the pivot can be done in $\mathcal{O}(1)$ by sampling 256 edges from the graph using OpenMP and taking the median thanks to Intel TBB (see experimental setup in Section \ref{sec:exp}).
...
\begin{itemize}
    ...
    \item Threading Building Blocks (TBB) for the sorting function that takes full advantage of multi-core performance.
    \item Parallel STL (PSTL) to have a threaded and vectorized execution of the standard C++ partition function, optimised and validated for Intel(R) 64 processors.
    \item LibSciBench (LSB) which is a framework containing a performance measurement library for parallel applications and is coupled with an R script for the post-processing of the gathered data \cite{benchmarking} \cite{measuring}. 
\end{itemize}
...
\mypar{Benchmarks} We tested our algorithms on three benchmarks :
\begin{itemize}
    \item Dense graphs : around 10K nodes and 10M edges with the topology of PA or ER.
    \item Sparse graphs : around 50K nodes and 2.5M edges with the topology of PA or ER.
    \item USA graphs : around 300K nodes and 800K edges with the topology of the USA road networks dataset.
\end{itemize}
...
\mypar{Baselines} We compare the results of our algorithms to those of the Parallel Boost Graph Library (PBGL) v1.62.0 which is an extension of the BGL for distributed computing.

\mypar{Difficulties}

\mypar{Results} We present here the results of our experiments on the benchmarks that we studied For every experiment, we generated a large number of random graphs on which three runs were made for each in order to have an appropriate standard deviation.

\section{Experimental Results 2}\label{sec:exp2}
...
\begin{figure}\noindent\centering
  \includegraphics[scale=0.28]{BoostKruskalSpeedup4.png}
  \caption{Boost Kruskal Speedup.\label{boostKruskal}}
\end{figure}
