...
In \cite{sollin} are described several parallel implementations of Sollin's algorithm and, notably, Bor-AL and Bor-FL that we present in Section \ref{sec:parSollin}. Both \cite{kruskal} and \cite{sollin} were an inspiration for the design of the Filter Sollin algorithm presented in Section \ref{sec:parSollin}.
...
\mypar{Filter Kruskal} The first parallel MST algorithm we consider is a variant of Kruskal's algorithm named Filter Kruskal (Algorithm \ref{algo:filterKruskal}).
...
\begin{itemize}
    ...
    \item Parallel STL to have a threaded and vectorized execution of the standard C++ partition function, optimised and validated for Intel(R) 64 processors.
\end{itemize}
...
%\begin{figure}\noindent\centering
%  \includegraphics[scale=0.3]{BoostKruskalSpeedup4.png}
